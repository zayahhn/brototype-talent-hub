    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        <Tabs defaultValue="complaints" className="w-full">
          ...
          <TabsContent value="students"><Card className="shadow-depth-lg"><CardHeader><CardTitle>Student Verification</CardTitle><CardDescription>Verify student profiles to make them visible to recruiters</CardDescription></CardHeader><CardContent>{students.length === 0 ? <p className="text-center text-muted-foreground py-8">No students yet</p> : <div className="space-y-4">{students.map((student) => (<Card key={student.id} className="border-border"><CardContent className="pt-6"><div className="flex items-start justify-between gap-4"><div className="space-y-2 flex-1"><div className="flex items-center gap-2"><h3 className="font-semibold">{student.profiles.name}</h3>{student.verified && <Badge variant="outline" className="text-success border-success"><CheckCircle className="h-3 w-3 mr-1" />Verified</Badge>}</div><p className="text-sm text-muted-foreground">{student.profiles.email}</p>{student.course && <p className="text-sm">Course: {student.course}</p>}{student.skills.length > 0 && <div className="flex flex-wrap gap-1">{student.skills.map((skill) => (<Badge key={skill} variant="secondary" className="text-xs">{skill}</Badge>))}</div>}</div>{!student.verified && <Button size="sm" onClick={() => handleVerifyStudent(student.id)} className="gap-2"><CheckCircle className="h-4 w-4" />Verify</Button>}</div></CardContent></Card>))}</div>}</CardContent></Card></TabsContent>
